...
try:
# if True:
    print("emd_approx_cuda")
    from PyTorchEMD.emd import earth_mover_distance
    print("print(emd_approx_cuda)")
    def emd_approx_cuda(sample, ref, require_grad=True):
...
def _pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size, accelerated_cd=True,
                      accelerated_emd=True,verbose=True):
    N_sample = sample_pcs.shape[0]
    N_ref = ref_pcs.shape[0]
    all_cd = []
    all_emd = []
...
def jsd_between_point_cloud_sets(sample_pcs, ref_pcs, resolution=28):
    ...
    """
    sample_pcs, ref_pcs = normalize_point_cloud_sets_for_jsd(
        sample_pcs, ref_pcs
    )
    in_unit_sphere = True
...
def normalize_point_cloud_sets_for_jsd(sample_pcs, ref_pcs):
    """Apply one shared scale that puts both sets inside radius 0.5.

    The dental coordinates are already expressed around the global origin.
    Scaling both distributions together preserves their relative position and
    scale differences. It also keeps masked zero-padding exactly at zero.
    """
    sample_pcs = np.asarray(sample_pcs)
    ref_pcs = np.asarray(ref_pcs)
    if sample_pcs.ndim != 3 or sample_pcs.shape[-1] != 3:
        raise ValueError(
            f'sample_pcs must have shape [S, P, 3], got {sample_pcs.shape}.'
        )
    if ref_pcs.ndim != 3 or ref_pcs.shape[-1] != 3:
        raise ValueError(
            f'ref_pcs must have shape [R, P, 3], got {ref_pcs.shape}.'
        )
    if not np.isfinite(sample_pcs).all() or not np.isfinite(ref_pcs).all():
        raise ValueError('JSD inputs contain NaN or Inf.')

    sample_radius = np.linalg.norm(sample_pcs, axis=-1).max(initial=0.0)
    ref_radius = np.linalg.norm(ref_pcs, axis=-1).max(initial=0.0)
    shared_radius = max(float(sample_radius), float(ref_radius))
    if shared_radius == 0.0:
        return sample_pcs.copy(), ref_pcs.copy()

    scale = 0.5 / shared_radius
    return sample_pcs * scale, ref_pcs * scale
...
def fps(pcd, K):
    """
    Sample K points from the point cloud using farthest point sampling.
    """
    pcd_tensor = torch.tensor(pcd, dtype=torch.float32).unsqueeze(0).cuda()
    sampled_points, _ = sample_farthest_points(pcd_tensor, K=K)
    return sampled_points.squeeze(0).cpu().numpy()



def read_masked_after_points(path):
    """Load a structured dental sample and canonicalize missing teeth."""
    with np.load(path) as sample:
        missing_keys = {
            key for key in ('after_pts', 'mask') if key not in sample
        }
        if missing_keys:
            raise KeyError(f'{path} is missing keys: {sorted(missing_keys)}')

        points = np.asarray(sample['after_pts']).copy()
        mask = np.asarray(sample['mask'])

    if points.shape != (32, 512, 3):
        raise ValueError(
            f'{path}: after_pts must have shape (32, 512, 3), '
            f'got {points.shape}.'
        )
    if mask.shape != (32,):
        raise ValueError(f'{path}: mask must have shape (32,), got {mask.shape}.')
    if not np.isfinite(points).all():
        raise ValueError(f'{path}: after_pts contains NaN or Inf.')

    mask = mask.astype(bool, copy=False)
    # Exact zeros are safe in evaluation and make real/fake padding identical.
    points[~mask] = 0.0
    return points, mask

if __name__=='__main__':
    # dataroot = '/data3/leics/dataset/teeth/old_gpt_segment_upper'
    # num = 100
    # num_points = 128
    # a = np.zeros([num, 16, num_points, 3])
    # for i in range(num):
    #     exist = False
    #     for j in range(16):
    #         data_path = f'{dataroot}/{i}_{j}.ply'
    #         if os.path.exists(data_path):
    #             exist = True
    #             pcd = read_pointcloud(data_path)
    #             sampled_pcd = fps(pcd, K=num_points)
    #             a[i, j] = sampled_pcd
    #     assert exist, f'not exist {i}'
    # a = torch.from_numpy(a).cuda().float()
    # a = a.reshape(num, -1, 3)

    # gt_dataroot = '/data3/leics/dataset/dental/pcd_normed'
    # b = np.zeros([num, 16, num_points, 3])
    # for i in range(num):
    #     exist = False
    #     for j in range(16):
    #         data_path = f'{gt_dataroot}/{i}_{j}.ply'
    #         if os.path.exists(data_path):
    #             exist = True
    #             pcd = read_pointcloud(data_path)[:512]
    #             sampled_pcd = fps(pcd, K=num_points)
    #             b[i, j] = sampled_pcd
    #     assert exist, f'not exist {i}'
    ...
    # a = torch.load('/data3/leics/dataset/gen_res5.pt').cuda()[:400].float()
    # b = torch.load('/data3/leics/dataset/gt.pt').cuda()[:400]
    # a = torch.randn([16, 4096, 3]).cuda()
    # b = torch.randn([16, 4096, 3]).cuda()


    ref_dataroot = '/data3/leics/dataset/teeth/sample512_merged'
    syn_dataroot = '/data3/leics/dataset/GenDental/old_gpt_stochastic_samples2'
    num = 100
    num_points = 128
    a = np.zeros([num, 32, num_points, 3])
    for i in range(num):
        data_path = f'{syn_dataroot}/{i}.npz'
        pcd, _ = read_masked_after_points(data_path)
        for j in range(32):
            sampled_pcd = fps(pcd[j], K=num_points)
            a[i, j] = sampled_pcd
    a = torch.from_numpy(a).cuda().float()
    a = a.reshape(num, -1, 3)

    b = np.zeros([num, 32, num_points, 3])
    for i in range(num):
        if i == 145:
            continue
        data_path = f'{ref_dataroot}/{i}.npz'
        pcd, _ = read_masked_after_points(data_path)
        for j in range(32):
            sampled_pcd = fps(pcd[j], K=num_points)
            if i>145:
