...
class TANet(nn.Module):
    ...
    def forward(self,centroid, before_points, after_points):
        '''
        points [bs,16,2048,3]
        '''
        bs = centroid.shape[0]
        ...
        encodings = []
        for i in range(n):
            encoding = self.encoders[i](before_points[:,i].permute(0,2,1))
        ...
        global_embedding = self.global_encoder(before_points.view(bs,-1,3).permute(0,2,1)).unsqueeze(1).repeat(1,n,1)
        center_emb = centroid.view(bs,-1).unsqueeze(1).repeat(1,n,1)
        before_embedding = torch.cat([global_embedding,trans_feature,center_emb],dim=-1)

        encodings = []
        for i in range(n):
            encoding = self.encoders[i](after_points[:,i].permute(0,2,1))
            encodings.append(encoding)
        for _ in range(2):
            encodings.append(torch.zeros_like(encodings[0]).cuda())
        embedding = torch.stack(encodings,dim=1)  #[bs,16,768]
        ...
        trans_feature = embedding[:,:32] + add_feature[:,:32]
        trans_feature = self.norm_layer(trans_feature)
        global_embedding = self.global_encoder(after_points.view(bs,-1,3).permute(0,2,1)).unsqueeze(1).repeat(1,n,1)
        center_emb = centroid.view(bs,-1).unsqueeze(1).repeat(1,n,1)
        after_embedding = torch.cat([global_embedding,trans_feature,center_emb],dim=-1)

        embedding = torch.cat([before_embedding,after_embedding],dim=-1)
        dofs = self.regressor(embedding)
        dofs = rearrange(dofs,'b n (l c) -> b l n c', c=9)
        return dofs