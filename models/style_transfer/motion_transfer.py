...
# from models.style_transfer.smooth import postprocess_smooth_se3
...
def smoothness_loss(predicted_matrices, masks, alpha=1.0, beta=1.0):
    ...
    loss_trans = trans_diff.mean(dim=1)  # 平均时间维

    # 相邻帧的旋转差（用 R_t^T * R_{t+1} 得到相对旋转）
...
def smooth_predicted_matrices_batch(predicted_matrices, window=5, poly=2):
    """
    对预测的刚体变换矩阵序列做后处理平滑（平移+旋转分开处理）
    
    Args:
        predicted_matrices: torch.Tensor, shape (b, l, p, 4, 4)
        window: Savitzky-Golay 滤波窗口大小 (奇数, 且 <= l)
        poly: Savitzky-Golay 多项式阶数
    Returns:
        smoothed: torch.Tensor, shape (b, l, p, 4, 4)
    """
    b, l, p, _, _ = predicted_matrices.shape
    device = predicted_matrices.device

    smoothed = np.tile(np.eye(4), (b, l, p, 1, 1))  # numpy 先初始化

    times = np.arange(l)  # 时间戳，用于 Slerp
...
class MotionTransfer(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        # # loss_smooth = (predicted_matrices[:,1:] - predicted_matrices[:,:-1]).abs().mean(dim=1).sum(dim=(-1,-2))
        # # loss_smooth = (loss_smooth * masks).mean()
        ...
        generated_points = torch.bmm(after_points - after_centroid.unsqueeze(1),rot_matrix) + (transition+after_centroid).unsqueeze(1)
        generated_points = rearrange(generated_points,'(b n) p c -> b n p c',n=32)

        criterion = nn.MSELoss(reduction='none')
        rec_loss = (criterion(generated_points,before_pts)*masks[:,:,None,None]).sum()
        loss = rec_loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print(rec_loss)
        # gt_points = rearrange(gt_points,'(b l p) n c -> b l (p n) c', p=p, l=l)
        # predicted_points = rearrange(predicted_points,'(b l p) n c -> b l (p n) c', p=p, l=l)
        # for i in range(b):
        #     # write_pointcloud(predicted_points[0,i].cpu().numpy(), f'/data3/leics/dataset/teeth/synthetic_tmp/{i}.ply')
        #     out_before_points = predicted_points[i][0].cpu().numpy()
        #     style_points = before_pts[i].cpu().numpy()
        #     out_after_points = after_pts[i].cpu().numpy()
        #     out_matrices = predicted_matrices[i].cpu().numpy()
        #     output_dir = '/data3/leics/dataset/teeth/synthetic_after_process_npz3'
        ...
        predicted_matrices[...,:3,3] = transition
        predicted_matrices[...,3,3] = 1
        predicted_matrices = postprocess_smooth_se3(predicted_matrices, masks)
